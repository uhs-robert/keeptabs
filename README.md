...
- `keeptabs-waybar` prints one Waybar JSON line per frame. Besides `text`, `class`, and `tooltip`, it emits a `finished_seq` field: a counter that goes up by the number of sessions that just went from running to done. A frontend other than Waybar can watch for it changing instead of diffing tooltip counts. For a few seconds after that transition, `class` also becomes an array with `finished` added (e.g. `["done", "finished"]`) so Waybar CSS can animate the change; see `.finished` in [`waybar-style.css`](examples/waybar-style.css). A `waiting_seq` field works the same way for sessions that just started waiting on you, pairing with a transient `waiting-started` class entry (e.g. `["waiting", "waiting-started"]`, or `["waiting", "finished", "waiting-started"]` if both fire at once); see `.waiting-started` and `.waiting` in [`waybar-style.css`](examples/waybar-style.css).
